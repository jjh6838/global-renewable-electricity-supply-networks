...
#SBATCH --output=outputs_per_country/logs/parallel_22_%j.out
#SBATCH --error=outputs_per_country/logs/parallel_22_%j.err
...
for ATTEMPT in $(seq 1 $MAX_RETRIES); do
    if $PY process_country_supply.py MMR $SCENARIO_FLAG --output-dir outputs_per_country; then
        echo "[SUCCESS] MMR completed (attempt $ATTEMPT)"
        break
    else
        if [ "$ATTEMPT" -lt "$MAX_RETRIES" ]; then
            echo "[WARN] MMR failed on attempt $ATTEMPT/$MAX_RETRIES - retrying in 10s..."
            sleep 10
        else
            echo "[ERROR] MMR failed after $MAX_RETRIES attempts"
        fi
    fi
done
...
for ATTEMPT in $(seq 1 $MAX_RETRIES); do
    if $PY process_country_supply.py NGA $SCENARIO_FLAG --output-dir outputs_per_country; then
        echo "[SUCCESS] NGA completed (attempt $ATTEMPT)"
        break
    else
        if [ "$ATTEMPT" -lt "$MAX_RETRIES" ]; then
            echo "[WARN] NGA failed on attempt $ATTEMPT/$MAX_RETRIES - retrying in 10s..."
            sleep 10
        else
            echo "[ERROR] NGA failed after $MAX_RETRIES attempts"
        fi
    fi
done
