...
for ATTEMPT in $(seq 1 $MAX_RETRIES); do
    if $PY process_country_supply.py ZAF $SCENARIO_FLAG --output-dir outputs_per_country; then
        echo "[SUCCESS] ZAF completed (attempt $ATTEMPT)"
        break
    else
        if [ "$ATTEMPT" -lt "$MAX_RETRIES" ]; then
            echo "[WARN] ZAF failed on attempt $ATTEMPT/$MAX_RETRIES - retrying in 10s..."
            sleep 10
        else
            echo "[ERROR] ZAF failed after $MAX_RETRIES attempts"
        fi
    fi
done
